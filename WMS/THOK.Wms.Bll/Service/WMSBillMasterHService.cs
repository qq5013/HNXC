...
namespace THOK.Wms.Bll.Service
{
    class WMSBillMasterHService : ServiceBase<WMS_BILL_MASTERH>, IWMSBillMasterHService
    {
        ...
        [Dependency]
        public IViewIncacheDateRepository ViewIncacheDateRepository { get; set; }
        [Dependency]
        public IViewProduceDateRepository ViewProduceDateRepository { get; set; }
        ...
        public object GetDetails(int page, int rows, string billtype, string flag, string BILL_NO, string BILL_DATE, string BTYPE_CODE, string WAREHOUSE_CODE, string BILL_METHOD, string CIGARETTE_CODE, string FORMULA_CODE, string STATE, string OPERATER, string OPERATE_DATE, string CHECKER, string CHECK_DATE, string STATUS, string BILL_DATEStar, string BILL_DATEEND, string SOURCE_BILLNO, string LINENO, string BILLNOFROM, string BILLNOTO)
        {
            try
            {
                ...
                IQueryable<VIEW_INCACHE_DATE> incachequery = ViewIncacheDateRepository.GetQueryable();
                IQueryable<VIEW_PRODUCE_DATE> datequery = ViewProduceDateRepository.GetQueryable();
                ...
                var billmaster = from a in billquery
                                 ...
                                 join h in incachequery on a.BILL_NO equals h.BILL_NO
                                 join j in datequery on a.BILL_NO equals j.BILL_NO
                                ...
                                && d.TABLE_NAME == "WMS_BILL_MASTER" && d.FIELD_NAME == "BILL_METHOD" && a.CMD_BILL_TYPE.BILL_TYPE == billtype
                                 select new
                                 {
                                     ...
                                     HFIRST_DATE=h.FIRST_DATE,
                                     HLAST_DATE=h.LAST_DATE,
                                     TFIRST_DATE=j.FIRST_DATE,
                                     TLAST_DATE = j.LAST_DATE
                ...
                var temp = billmaster.ToArray().Select(i => new
                {
                    ...
                    i.LINE_NO,
                    i.LINE_NAME,
                    HFIRST_DATE = i.HFIRST_DATE == null ? "" : ((DateTime)i.HFIRST_DATE).ToString("yyyy-MM-dd HH:mm:ss"),
                    HLAST_DATE = i.HLAST_DATE == null ? "" : ((DateTime)i.HLAST_DATE).ToString("yyyy-MM-dd HH:mm:ss"),
                    TFIRST_DATE = i.TFIRST_DATE == null ? "" : ((DateTime)i.TFIRST_DATE).ToString("yyyy-MM-dd HH:mm:ss"),
                    TLAST_DATE = i.TLAST_DATE == null ? "" : ((DateTime)i.TLAST_DATE).ToString("yyyy-MM-dd HH:mm:ss")
                });
        ...
        //入库查询
        public object GetDetailsIn(int page, int rows, string billtype, string flag, string BILL_NO, string BILL_DATE, string BTYPE_CODE, string WAREHOUSE_CODE, string BILL_METHOD, string CIGARETTE_CODE, string FORMULA_CODE, string STATE, string OPERATER, string OPERATE_DATE, string CHECKER, string CHECK_DATE, string STATUS, string BILL_DATEStar, string BILL_DATEEND, string SOURCE_BILLNO, string LINENO, string BILLNOFROM, string BILLNOTO)
        {
            try
            {
                IQueryable<VIEW_BILL_MAST> billquery = viewbillmastRepository.GetQueryable();
        ...
        //批次校验
        public bool CheckBill(string billno, out string outinfo)
        {
            //string resultinfo="";
            string exestorestr = "begin CHECKCELLBYBILL('" + billno + "');end;";
            int result = BillMasterHRepository.Exeprocedure(exestorestr, out outinfo);
            if (result < 0)
                return false;
            else
            {
