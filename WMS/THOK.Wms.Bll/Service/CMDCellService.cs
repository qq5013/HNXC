...
namespace THOK.Wms.Bll.Service
{
    public class CMDCellService : ServiceBase<CMD_CELL>, ICMDCellService
    {
        ...
        [Dependency]
        public IViewbillmastRepository viewbillRepository { get; set; }
        [Dependency]
            ...
            if (cellitem.Count() > 0)//有存在该货位
            {
                foreach (CMD_CELL cell in cellitem)
                {
                    ...
                    if (cell.PRODUCT_CODE  != null)
                    {
                        ...
                        var bill = viewbillRepository.GetQueryable().FirstOrDefault(i => i.BILL_NO == cell.BILL_NO);
                        if (bill != null)
                        {
        ...
        //禁用层
        public int disablelayer(string shelfcode, int rowcode,int flag,out string error)
        {
            int result=1; //0表示用户所要禁的货位 存在is_lock为1的货位 需要让用户在确认；1表示可以直接执行任务,并且执行成功;-1表示任务执行错误
            error = "执行成功";
            if (flag == 0) //用于判断，并通知用户确认
            {
                var isexit = CMDCellRepository.GetQueryable().Where(i => i.SHELF_CODE == shelfcode && i.CELL_ROW == rowcode && i.IS_LOCK == "1");
                if (isexit.Count() > 0)
                {
                    result = 0;
                }
            }
            if (result == 1||flag ==1) //执行任务
            {
                string sqlstr = "begin DISABLECELLROW('" + shelfcode + "','" + rowcode + "');end;";
                int isok = CMDCellRepository.Exeprocedure(sqlstr,out error);
        ...
        //启用层
        public bool undisabalelayer(string shelfcode, int rowcode, out string error)
        {
            error = "执行成功";
            string sqlstr = "begin UNDISABLECELLROW('" + shelfcode + "','" + rowcode + "');end;";
            int isok = CMDCellRepository.Exeprocedure(sqlstr, out error);
            //return ((ObjectContext)RepositoryContext).ExecuteStoreCommand("","");
            if (isok < 0)
                return false;
