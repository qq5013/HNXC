...
namespace THOK.Wms.Bll.Service
{
    class CMDProductGradeService : ServiceBase<CMD_PRODUCT_GRADE >, ICMDProductGradeService
    {
        ...
        [Dependency]
        public IWMSFormulaDetailRepository formulaRepository { get; set; }
        [Dependency]
        ...
        public object Detail(int page, int rows, string ENGLISH_CODE, string USER_CODE, string GRADE_NAME, string MEMO)
        {
            ...
            temp = temp.Where(i => i.GRADE_CODE != "000");
        ...
        public object Detail(int page, int rows, string formula)
        {
            IQueryable<CMD_PRODUCT_GRADE> query = ProductGradeRepository.GetQueryable();
            var temp = query.OrderBy(i => i.ENGLISH_CODE).Select(i => new { i.GRADE_CODE, i.ENGLISH_CODE, i.USER_CODE, i.GRADE_NAME, i.MEMO });
            if (!string.IsNullOrEmpty(formula))
            {
                var formulaquery = formulaRepository.GetQueryable();
                var productquery=ProductRepository .GetQueryable ();
                temp = temp.Where(i =>(from a in formulaquery join b in productquery on a.PRODUCT_CODE equals b.PRODUCT_CODE  where a.FORMULA_CODE ==formula select b.GRADE_CODE).Contains (i.GRADE_CODE ));
            }
            int total = temp.Count();
            temp = temp.Skip((page - 1) * rows).Take(rows);
            return new { total, rows = temp.ToArray() };
