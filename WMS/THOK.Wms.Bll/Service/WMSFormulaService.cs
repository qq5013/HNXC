    ...
    public class WMSFormulaService:ServiceBase<WMS_FORMULA_MASTER>, IWMSFormulaService
    {
        ...
        public object GetDetails(int page, int rows, string BTYPE_NAME, string BILL_TYPE, string TASK_LEVEL, string Memo, string TARGET_CODE, string FORMULA_CODE, string FORMULA_NAME, string CIGARETTE_CODE, string ISACTIVE, string FORMULADATE, string OPERATER)
        {
            ...
            var formulamast = from a in masterQuery
                              join b in userquery on a.OPERATER equals b.USER_ID into fg from b in fg.DefaultIfEmpty()
                              select new {
                                 a.FORMULA_CODE,
                                  a.FORMULA_DATE,
                                  a.FORMULA_NAME,
        ...
        public object GetSubDetails(int page, int rows, string FORMULA_CODE)
        {
            ...
            var Details = DetailQuery.OrderBy(i => i.FORDER).ThenBy(i => i.PRODUCT_CODE).Select(i => new
            {
                  ...
                  ORIGINAL= i.CMD_PRODUCT .CMD_PRODUCT_ORIGINAL .ORIGINAL_NAME ,
        ...
        public bool Delete(string FORMULA_CODE)
        {
            try
            {
                if (!string.IsNullOrEmpty(FORMULA_CODE))
                {
                    string[] formulcode = FORMULA_CODE.Split(';');
                    for (int n = 0; n < formulcode.Length-1; n++)
                    {
                        string itemcode = formulcode[n];
                        var editmaster = MasterRepository.GetQueryable().Where(i => i.FORMULA_CODE == itemcode).FirstOrDefault();

                        var details = DetailRepository.GetQueryable().Where(i => i.FORMULA_CODE == itemcode );
                        var tmp = details.ToArray().AsEnumerable().Select(i => i);
                        foreach (WMS_FORMULA_DETAIL sub in tmp)
                        {
                            DetailRepository.Delete(sub);
        ...
        //根据配方获取对应的模块号
        public object Getmodules(string FORMULACODE)
        {
            var formuladetail = DetailRepository.GetQueryable();
            var modules=from a in formuladetail 
                        where a.FORMULA_CODE ==FORMULACODE
                        group a by new { a.MODULES} into g
                        select new
                        {
                            g.Key .MODULES
                        };
            int total = modules.Count();
            var temp = modules.ToArray().Select(i => new
            {
                i.MODULES 
            });
