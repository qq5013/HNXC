...
namespace THOK.XC.Process.Dao
{
    public class SysStationDao : BaseDao
    {
        ...
        /// <returns></returns>
        public string GetToStation(string FromStation)
        {
            string strSQL = string.Format("SELECT MEMO FROM SYS_STATION t WHERE t.station_type='22' and t.station_no='{0}'", FromStation);
            DataTable dt = ExecuteQuery(strSQL).Tables[0];
            if (dt.Rows.Count > 0)
