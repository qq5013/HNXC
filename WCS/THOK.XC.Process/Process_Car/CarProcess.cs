...
namespace THOK.XC.Process.Process_Car
{
    public class CarProcess : AbstractProcess
    {
        ...
        private void GetTaskList()
        {
            ...
            string strSort = "TASK_TYPE desc,FORDER,TARGET_CODE,Index,STATION_NO desc";
            if (PalletCount >= 3)
            ...
            else
            {
                ...
                int gsTargetIdx = dal.GetCarTargetSeq();
                switch (gsTargetIdx)
                {
                    case 2:
                        strSort = "TASK_TYPE desc,FORDER,TARGET_CODE desc,STATION_NO desc";
                        break;
                    default:
                        strSort = "TASK_TYPE desc,FORDER,TARGET_CODE,Index,STATION_NO desc";
                        break;
                }
            }
        ...
        private void CarStateChange(string CarNO)
        {
            ...
            {                 
                if (dCarStatus[CarNO].State == "2") //送货完成
                {
                    ...
                    string[] strStationNo = new string[2];
                    ...
                    if (CarTaskNo != "9999" && CarTaskNo != "0")
                    {
                        ...
                        strStationNo = dal.GetCarToStation(CarNO, CarTaskNo);                        

                        if (strStationNo.Length <= 0)
                        {
                            ...
                            {
                                strStationNo[0] = drsAddress[0]["STATION_NO"].ToString();
                                strStationNo[1] = drsAddress[0]["PROCESS_NAME"].ToString();
                            }
                        }

                        string strItemState = "02_1_" + strStationNo[0];
                        string strItemName = strStationNo[1];
                        ...
                        if (strItemName.Length > 0)
                        {
                            ...
                            Logger.Info("小车" + CarNO + "送货完成;任务号:" + CarTaskNo + ",没找到对应的站台号,无法下达任务");
                    }
