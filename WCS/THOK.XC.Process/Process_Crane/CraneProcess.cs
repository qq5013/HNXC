    ...
    public class CraneProcess : AbstractProcess
    {
        ...
        private void Crane_Out(int piCrnNo)
        {
            ...
            for (int i = 0; i < 3; i++)
            {
                ...
                {
                    // 找出此堆垛机所有出库任务
                    string CraneNo = Convert.ToString(piCrnNo).PadLeft(2, '0');
                    DataTable tempDt = taskDal.GetOutTasks(CraneNo, gsTarget);
                    ...
                    if (tempDt.Rows.Count > 0)
                    {
                        ...
                        {
                            Logger.Debug("出库任务插入内存是发生错误:" + e.Message.ToString());
                            return;
                        }
                        ...
                        if (tempDt.Rows[0]["TASK_TYPE"].ToString() == "22") // 2F 出库
                        {
                            ...
                            {
                                //判断是否能出库                 
                                if (!taskDal.ProductOutToStation(tempDt.Rows[0]["FORDERBILLNO"].ToString(), int.Parse(tempDt.Rows[0]["FORDER"].ToString()), CraneNo, blnOutOrder))
                                {
                            ...
                            {
                                Logger.Debug("判断能否出库时发生错误:" + e.Message.ToString());
                                return;
                            }
                        }
                        ...
                        {
                            Logger.Debug("给堆垛机发送报文时发生错误:" + e.Message.ToString());
                            return;
                        }
                    }
                }
        ...
        {
            //读取出库站台有无货物,如有直接返回
            object obj = ObjectUtil.GetObject(WriteToService(psServiceName, psItemName));
            if (obj == null)
                return false;
        ...
        private void CSR(object state)
        {
            try
            {
                ...
                if (msg == null)
                {
                    Logger.Debug("Csr state 返回为null");
                    return;
                }
