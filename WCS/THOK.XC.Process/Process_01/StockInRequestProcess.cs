    ...
    public class StockInRequestProcess : AbstractProcess
    {
        protected override void StateChanged(StateItem stateItem, IProcessDispatcher dispatcher)
        {
            ...
            try
            {
                ...
                //strWhere = string.Format("SUBSTR(PRODUCT_BARCODE,1,64)='{0}' AND CELL_CODE IS NULL AND STATE=0", BarCode);
                string strWhere = "";
                //if (TaskID == "")
                //    strWhere = string.Format("PRODUCT_BARCODE='{0}' AND CELL_CODE IS NULL AND STATE=0", BarCode);
                //else
                //    strWhere = string.Format("TASK_ID='{0}'", TaskID);

                if (TaskID == "")
                    strWhere = string.Format("A.PRODUCT_BARCODE='{0}'", BarCode);
                else
                    strWhere = string.Format("A.TASK_ID='{0}'", TaskID);

                //Logger.Info("申请位置:" + stateItem.ItemName + ";申请条码:" + BarCode);
                
                TaskDal dal = new TaskDal();
                //分配货位,返回 0:TaskID，1:货位 
                string[] CellValue = dal.AssignCell(strWhere, ToStation);//货位申请
                //返回任务号9999
                string TaskNo = CellValue[2];
                if (TaskNo.Trim().Length <= 0)
                    TaskNo = dal.InsertTaskDetail(CellValue[0]);
                ...
                {
                    byte[] b = new byte[90];
                    Common.ConvertStringChar.stringToByte(dtTask.Rows[0]["PALLET_CODE"].ToString(), 90).CopyTo(b, 0);
                    //写入RFID
                ...
                Logger.Info("任务号：" + TaskNo + " 货位：" + CellValue[1]);
            }
            catch (Exception e)
            {
